  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select clearable class="filter-item" style="width: 130px" v-model="listQuery.type" :placeholder="$t('table.type')">
        <el-option v-for="item in  enableOptions" :key="item.key" :label="item.display_name+'('+item.key+')'" :value="item.key">
        </el-option>
      </el-select>
    ...
    <el-table :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      style="width: 100%">
      <!--ID-->
      <el-table-column align="center" :label="$t('role.id')" width="65">
      ...
      <!--描述-->
      <el-table-column align="center" :label="$t('role.roleDescription')" min-width="65px">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" :label="$t('role.actions')" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.enable!='deleted'" size="mini" type="danger" @click="handleDelete(scope.row)">{{$t('table.delete')}}
          </el-button>
        </template>
      </el-table-column>
      <!--创建时间-->
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="100px" style='width: 400px; margin-left:50px;'>
        <el-form-item :label="$t('role.roleCode')" prop="code">
          <span v-if="dialogStatus=='delete'">{{temp.code}}</span>
          <el-input v-else v-model="temp.code"></el-input>
        </el-form-item>
        <el-form-item :label="$t('role.roleDescription')" prop="name">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button v-else-if="dialogStatus=='delete'" type="primary" @click="deleteData">{{$t('table.confirm')}}</el-button>
      </div>
    </el-dialog>
