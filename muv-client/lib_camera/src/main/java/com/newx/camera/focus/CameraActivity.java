...
public class CameraActivity extends Activity {
    ...
    void initView() {
        ...
        m_ibRecentPic = findViewById(R.id.ib_recentpic);
    }
    ...
    void initData() {
        ...
        if (list != null && list.size() != 0) {
            ...
            m_ibRecentPic.setOnClickListener(v -> {
                //跳转到系统相册
    ...
    void initListener() {
        if (mCameraManager.canSwitch()) {
            m_tvCameraDireation.setOnClickListener(v -> {
                m_tvCameraDireation.setClickable(false);
                mCameraContainer.switchCamera();

                //500ms后才能再次点击
                handler.postDelayed(() -> m_tvCameraDireation.setClickable(true), 500);
            });
        }

        m_tvFlashLight.setOnClickListener(v -> mCameraContainer.switchFlashMode());
    }
