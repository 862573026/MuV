...
import com.newx.camera.def.CameraKey;
import com.newx.camera.def.CameraPath;
import com.newx.utils.mobile.SPUtil;
import com.newx.utils.utilcode.util.FileUtils;
import com.newx.utils.utilcode.util.ScreenUtils;
...
public class CameraManager implements ICameraHelper {
    ...
    private TextView m_tvFlashLight, m_tvCameraDirection;  //绑定的闪光灯和前后置镜头切换控件

    public static List<FlashLightStatus> mFlashLightNotSupport = new ArrayList<FlashLightStatus>();
    ...
    private CameraManager(Context context) {
        ...
        mLightStatus = FlashLightStatus.valueOf((Integer) SPUtil.get(CameraKey.KEY_LIGHT_STATUE, FlashLightStatus.LIGHT_AUTO.ordinal())); //默认 自动
        mFlashDirection = CameraDirection.valueOf((Integer) SPUtil.get(CameraKey.KEY_CAMERA_DIRECTION, CameraDirection.CAMERA_BACK.ordinal())); //默认后置摄像头
        FileUtils.createOrExistsDir(CameraPath.PHOTO_DIR);
    }
    ...
    public void setLightStatus(FlashLightStatus mLightStatus) {
        ...
        if (m_tvFlashLight != null) {
            ...
            SPUtil.put(CameraKey.KEY_LIGHT_STATUE, mLightStatus.ordinal());
        }
    }
    ...
    public void setCameraDirection(CameraDirection mFlashDirection) {
        ...
        // 保留用户设置
        if (m_tvCameraDirection != null) {
            m_tvCameraDirection.setText(RES_STRING_CAMERA_DIRECTION[mFlashDirection.ordinal()]);

            Drawable drawable = mContext.getResources().getDrawable(RES_DRAWABLE_CAMERA_DIRECTION[mFlashDirection.ordinal()]);
            drawable.setBounds(0, 0, LEN_PIC, LEN_PIC);
            m_tvCameraDirection.setCompoundDrawables(drawable, null, null, null);
            //不再记录相机方向  会导致部分相机 前置摄像头
            SPUtil.put(CameraKey.KEY_CAMERA_DIRECTION, mFlashDirection.ordinal());

            m_tvFlashLight.setVisibility(mFlashDirection == CameraDirection.CAMERA_BACK ? View.VISIBLE : View.INVISIBLE);
        }
    }

    public void setCameraDirButtonClickable(boolean clickable) {
        if (m_tvCameraDirection != null) {
            m_tvCameraDirection.setClickable(clickable);
        }
    }
    ...
    public void bindOptionMenuView(TextView tvFlashLight, TextView tvCameraDirection) {
        ...
        m_tvCameraDirection = tvCameraDirection;
        ...
        //设置监听
//        if (m_tvFlashLight != null) {
//            m_tvFlashLight.setOnClickListener(v -> setLightStatus(getLightStatus().next()));
//        }
    }

    public void unbinding(){
        m_tvCameraDirection = null;
        m_tvFlashLight = null;
    }
