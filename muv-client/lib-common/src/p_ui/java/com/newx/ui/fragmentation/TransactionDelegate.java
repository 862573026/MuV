package com.newx.ui.fragmentation;
...
import com.newx.ui.fragmentation.helper.internal.ResultRecord;
import com.newx.ui.fragmentation.helper.internal.TransactionRecord;
...
public class TransactionDelegate {
    ...
    private void startWithPop(final FragmentManager fragmentManager, final ISupportFragment from, final ISupportFragment to) {
        if (FragmentationHack.isExecutingActions(fragmentManager)) {
            mHandler.post(() -> executeStartWithPop(fragmentManager, from, to));
            return;
        }
    ...
    void handleResultRecord(Fragment from) {
        ...
        mHandler.post(() -> preFragment.onFragmentResult(resultRecord.requestCode, resultRecord.resultCode, resultRecord.resultBundle));
    }
    ...
    void popTo(final String targetFragmentTag, final boolean includeTargetFragment, final Runnable afterPopTransactionRunnable, FragmentManager fragmentManager, final int popAnim) {
        ...
        if (FragmentationHack.isExecutingActions(fragmentManager)) {
            ...
            mHandler.post(() -> executePopTo(targetFragmentTag, includeTargetFragment, afterPopTransactionRunnable, finalFragmentManager, popAnim));
            return;
        }
    ...
    private void mockStartWithPopAnim(ISupportFragment from, ISupportFragment to, final Animation exitAnim) {
        ...
        to.getSupportDelegate().mEnterAnimListener = () -> {
            ...
            mHandler.postDelayed(() -> container.removeView(mock), exitAnim.getDuration() + BUFFER_TIME);
        };
    }
