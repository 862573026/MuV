...
public class RxBus {
    ...
    public static final int THREAD_MAIN   = 0x00000001;//主线程
    public static final int THREAD_IO     = 0x00000002;// 子线程
    ...
    public void postSticky(@NonNull Object o){
        postSticky(o.getClass().getName(), o);
    }

    // 给同属同一tag的事件群发消息(如果该tag未注册，则缓存) - 类似粘性广播
    public void postSticky(@NonNull Object tag, @NonNull Object o){
        List<Subject> subjects = maps.get(tag);
    ...
    public void postDelay(@NonNull Object tag, @NonNull Object o, long delayMillis){
        List<Subject> subjects = maps.get(tag);
        Subject singleSubject = singleMaps.get(tag);
        boolean hasValue = false;
        if (subjects != null && !subjects.isEmpty()){
