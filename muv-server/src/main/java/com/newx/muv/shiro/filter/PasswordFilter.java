package com.newx.muv.shiro.filter;
...
import com.newx.muv.util.CommonUtil;
import com.newx.muv.util.IpUtil;
...
public class PasswordFilter extends AccessControlFilter {
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (isAccountLogoutPost(request)) {
            return true;
        }
        ...
        // response 告知无效请求
        Message message = new Message().error(RespCode.ERROR, "error request");
        RequestResponseUtil.responseWrite(JSON.toJSONString(message), response);
        return false;
    }
...
//        String uid = map.get("uid");
        ...
        String methodName = ((ShiroHttpServletRequest) request).getRequestURI();
        String password = map.get("password");
        return (request instanceof HttpServletRequest)
//                && ((HttpServletRequest) request).getMethod().toUpperCase().equals("POST")
                && null != username
...
//                && null != uid
    ...
    private boolean isAccountLogoutPost(ServletRequest request) {
        ...
        return (request instanceof HttpServletRequest)
                && null != methodName
                && null != uid
                && methodName.equals("/account/logout");
    }
